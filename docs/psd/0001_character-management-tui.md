...
| D25 | Phase 0 scope | **Narrowed again in v0.6.** Beyond `play_log` it carries only the attunement default that the generated types inject on decode, plus the `schema_version` bump and regeneration. The `additionalProperties: false` tightening v0.5 folded in is withdrawn — the schema stays open through `v1alpha` (D26). One additive format change. See ARD 0003, ARD 0004, ARD 0011. | §6.1, §11 |
| D26 | Schema openness | **`additionalProperties` stays open throughout the character schema for the life of `v1alpha`**, the root included. Adding a field must not invalidate documents written before it; an unrecognized field is unsupported data, not an error. This makes preserving unknown keys on save an obligation on the write path. See ARD 0011. | §6, §9.3 |
| D27 | `dmostd` | **Frozen until LAN server work begins.** It keeps building and its tests keep passing; it gains no features and stops obliging `internal/infra/inmem` to implement the whole widened port. See ARD 0010. | §5.3, §11 |
...
**Why it must land first.** D9 makes the Go type tree a build artifact of the schema. A root-level field added after Phase 1 means regenerating types, re-running every screen's model against them, and revisiting the round-trip corpus — for a field that could have been there from the first commit at no cost. v0.4.7 added a second reason — that the root's `additionalProperties: false` makes a document carrying an unrecognised field *invalid*, so the tool could not tolerate it in the meantime — and **v0.6 withdraws it (D26)**: the declaration was never enforced, and the root is now open by decision. The regeneration argument above carries this on its own.
...
**Phase 0 carries one more schema change with it (D25).** It was found by auditing the generated types against the schema, it is a silent-loss bug rather than a feature, and it wants the same single format bump: the attunement default, where `combat.attunement.slots_maximum` is injected as 3 on decode into a field tagged `omitzero`, so opening a sheet that omitted it and letting autosave fire writes it in with no user edit. §11 sequences it.

v0.5 also folded in `additionalProperties: false` throughout the schema, on the reasoning that a schema-legal extra key inside `combat` or `inventory.entries[]` is dropped or kept today depending only on whether the generator emitted a struct or a map. **That is withdrawn in v0.6 (D26).** Closing the schema makes every later field addition invalidate the documents written before it, which is the wrong trade while the format is still moving. The measured loss is real and does not go away — it is reassigned to the save path, which must preserve keys the generated tree has no field for. ARD 0011 carries the decision, the obligation, and the mechanism.
...
*The schema (D25).* The `play_log` addition (§6.1), the attunement default, the `schema_version` bump to `1.1.0`, the three remaining closed objects opened (D26), and regenerated types. One format change rather than three, so that the codegen and the round-trip corpus are built against the final root shape rather than migrated to it later. `play_log` also joins `serverOwnedSheetKeys`, which touches `internal/dto/v1alpha/mapper` and its tests.

*The stack (D22).* The aggregate grows an item set and an `ItemInstance` entity; the repository port grows from two methods to nine; `pkg/app` moves to version-neutral command structs and gains the two write use cases; validation moves to the compiled schema; the SQLite adapter and `repotest` grow with the port, while `inmem` grows only where something calls it (D27); §9.1's migrations land. This breaks signatures across `pkg/domain/character`, `pkg/app`, `internal/dto/v1alpha/mapper`, `internal/infra`, `internal/test`, `pkg/http` and `cmd/dmostd` — cheaply, because nothing has shipped and no database has ever survived a process exit, and never as cheaply again. No behaviour and no screens come out of either strand.
